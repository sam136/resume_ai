...
export class ResumeController {
  ...
  async getResumes(req: Request, res: Response) {
    try {
      ...
      const resumes = await Resume.find(query.filter || {})
        .sort(query.sort);
  ...
  async getResumeById(req: Request, res: Response) {
    try {
      ...
      const resume = query.populate ? await query_result.populate(query.populate) : await query_result;
      if (!resume) {
  ...
  async exportResume(req: Request, res: Response) {
    try {
      ...
      const exportedFile = await exportResumeToPDF(toJSON(resume) as unknown as ResumeData);
  ...
  async parseResume(req: Request, res: Response) {
    try {
      ...
      const resumeData = this.formatForDatabase(parseResult, req.user?.id || 'anonymous');
  ...
  private formatForDatabase(parseResult: ParsedResume, userId: string): Partial<ResumeData> {
